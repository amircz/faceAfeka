...
function getUserPic($user_id)
{
	...
	$res=@mysqli_query($sql,"SELECT img FROM users WHERE user_id='".$user_id."'") or die("<BR>ERROR: incorrect query!");
...
function GetPost($username, $user_id, &$posts)
{
	...
	$res=@mysqli_query($sql,"SELECT distinct posts.post_id, posts.user_id, posts.date, posts.private,
					   ...
					   ORDER BY date DESC
					   LIMIT 8") or die("<BR>ERROR: incorrect query!");
	//if we got a row, then we got a match
	if (mysqli_num_rows($res)>0)
	{
...
{
	$sqlQ=@mysqli_connect("localhost","root","") or die("<BR>ERROR: cannot connect to MySQL server!");  //connect to MySQL server
	@mysqli_select_db($sqlQ,"faceafeka") or die("<BR>ERROR: cannot use the DB!");  //selects the DB
	
	$result = @mysqli_query($sqlQ,"SELECT comments.comment_id, comments.post_id, comments.user_id, comments.content,
							comments.date FROM comments
					  		WHERE comments.post_id = '".$postId."' ORDER BY date DESC") or die("<BR>ERROR: incorrect query!");
	...
	if (mysqli_num_rows($result)>0)
	{
		while ($com_row=mysqli_fetch_array($result))  //as long as there are rows in the result
		{
			...
			$name1 = @mysqli_query($sqlQ,"SELECT users.userName FROM users WHERE users.user_id = '".$user_id."'") or die("<BR>ERROR: incorrect query!");
