	...
	foreach ($posts as $post) {
				...
				($post->getPost_username() != $username && !$post->getPrivate()))
		{
			...
			if($post->getcomments_num()>0)
				...
				//content
				echo "<tr>
					   <td style = 'word-wrap:break-word; max-width: 330px;'><h3><font color = 'green'>".$post->getContent()."</font></h3><hr></td>
				  </tr>";
				...
				echo "<td> <a class = 'hideLikes'>
							  ...
							  <input type= 'image' src = 'images/like.png' class='like' value='".$post->getLikes_num()."' id='like' name = '".$post->getPost_id()."' height = '30px' width = '30px'></input></td>
				...
				if($post->getPost_user_id() != $_SESSION["user_id"])
					...
					echo "</FORM>";
					if($post->getPost_user_id() == $_SESSION["user_id"])	
						echo "<tr><td><input type = 'image' src = 'images/perm.png' id = 'perm' class= 'perm' name = '".$post->getPost_id()."' width = '150px' height ='30px' ></input></td></tr>";
					...
					echo "<tr>
								<td>
									...
									<div id = 'comDiv'>
										<p id = 'show' name = '".$post->getPost_id()."Show' ><u><font color = 'green'>".getCommentsTxt($post->getcomments_num())."</font></u></p>	
									</div>";
					...
					echo  "<tr align = 'left'>
								<td>
									<div id = 'comm'>
										<p><font color= 'green'>New comment:</font></p></td></tr>
										<tr align = 'right'><td>
										<input type='hidden' name='post_id' id='post_id' value='".$post->getPost_id()."'>
										<textArea class = 'commCont' id ='commCont' rows='4' cols = '50' name = '".$post->getPost_id()."'></textArea>
										<input type = 'image' src = 'images/post.png' id = 'post' class='post' height = '30px' width = '100px' name = '".$post->getPost_id()."'></input>
									</div>
								</td>
								</tr>";
					if($post->getcomments_num()>0)
...
{
	$sqlQ=@mysqli_connect("localhost","root","") or die("<BR>ERROR: cannot connect to mysqli server!");  //connect to mysqli server
	@mysqli_select_db("faceafeka",$sqlQ) or die("<BR>ERROR: cannot use the DB!");  //selects the DB
...
<html>
	...
	<div id = "header">	
		<table style = "align-self: center; border:1; height: 50px;">
			<tr>
				<td width = "45px"><img src= "images/logo.png" height = "45px" width="45px"></td>
				<td><img src= "images/fa2.png" height = "60px" width="80px"></td>
				...
				<td>				
					<form action="logout.php" method="POST" id="formLog">
						<div id="submitForm">
							<input type="image" src = 'images/logout' height = "45px" width = "45px"  name = "log" id = "log"/>
						</div>
					</form >		
				</td>
			</tr>	
	...
	<table align="center">	
		...
		<tr align="center">
			<td><font color = "green">Insert images to post:</font>
			<input type="file" name="files[]" id ="files[]"  multiple />
			</td>
		</tr>
		<tr align="center">
			<td><font color = "green">Add Check In?</font>
				<input type = "radio" name = "check" value = "no" checked id = "no"><font color= "green">No</font></input>
				<input type = "radio" name = "check" value = "yes" id = "yes"><font color= "green">Yes</font></input>
			</td>
		...
		<tr align="center">
			<td>
				<p>
					<input type = "radio" name = "radio" value = "public" checked id = "public"><font color= "green">Public</font></input>
					<input type = "radio" name = "radio" value = "private" id = "private"><font color= "green">Private</font></input>
					<input type="image" src = "images/postA.png" value = "Post" name = "addPostBtn" id = "addPostBtn" width = "70px" height= "30px"/></p>
			</td>			
		</tr>	
	</table>
	</FORM>
